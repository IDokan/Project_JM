...
public class LeaderboardRow : MonoBehaviour, ISelectHandler
{
    ...
    private void Awake()
    {
        _autoScroll = GetComponentInParent<ScrollRectAutoScroll>();
        _navigation = GetComponentInParent<LeaderboardNavigation>();
    }

    public void Initialize(bool isMyRecord, Sprite icon0, Sprite icon1, Sprite icon2, int score, System.Action<Button> onSelected = null)
    {
    ...
    {
        _autoScroll?.OnItemSelected(GetComponent<RectTransform>());
